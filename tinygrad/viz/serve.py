...
def uop_to_json(x:UOp) -> dict[int, tuple[str, list[int], str]]:
  ...
  for u in (toposort:=x.toposort):
    ...
    if u.op is Ops.CONST and len(u.src) != 0 and all(cr.op is Ops.CONST for c in u.src[0].children if (cr:=c()) is not None and cr in toposort): excluded.update(u.src)
  for u in toposort:
